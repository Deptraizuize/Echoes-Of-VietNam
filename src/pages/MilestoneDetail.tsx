...
const ReadingProgress = () => {
  ...
  return <motion.div className="fixed top-0 left-0 right-0 h-[2px] bg-accent origin-left z-[60]" style={{ scaleX }} />;
};

/* Gallery carousel - extracted */
    ...
    <div className="relative select-none">
      <div className="space-y-0 rounded-2xl overflow-hidden border border-border bg-card shadow-heritage">
        <div className="relative aspect-[16/10] sm:aspect-[16/9] overflow-hidden bg-muted">
          <AnimatePresence mode="popLayout">
            ...
            >
              <img src={images[current]} alt={captions[current] || `Tư liệu ${current + 1}`} className="w-full h-full object-cover" loading="lazy" />
            </motion.div>
          </AnimatePresence>
        ...
        <div className="border-t border-border bg-card px-5 py-4 min-h-[3.5rem] flex items-center gap-4">
          <AnimatePresence mode="wait">
            <motion.p key={`cap-${current}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.25 }} className="text-muted-foreground text-sm md:text-base italic leading-relaxed flex-1">
          ...
          {count > 1 && count <= 8 && (
            <div className="flex gap-1.5 shrink-0">
              {images.map((_, i) => (
                <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === current ? "bg-accent w-5" : "bg-border hover:bg-muted-foreground/40"}`} />
              ))}
            </div>
          )}
        </div>
      ...
      {count > 1 && (
        <div className="flex gap-2 mt-4 justify-center overflow-x-auto py-1 px-2 scrollbar-none">
          {images.map((url, i) => (
            <button key={i} onClick={() => setCurrent(i)} className={`relative shrink-0 w-14 h-10 sm:w-[4.5rem] sm:h-12 rounded-lg overflow-hidden border-2 transition-all duration-300 ${i === current ? "border-accent shadow-gold-glow scale-105" : "border-transparent opacity-40 hover:opacity-75 grayscale hover:grayscale-0"}`}>
        ...
        <div className="relative h-[45vh] md:h-[55vh]">
          ...
          <div className="absolute top-4 left-4 md:top-6 md:left-8 z-10">
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
              <Button variant="ghost" size="sm" onClick={() => navigate("/timeline")} className="bg-black/30 backdrop-blur-sm text-white/80 hover:text-white hover:bg-black/50 border border-white/10">
          ...
          {detail ? (
            <div className="space-y-16 md:space-y-20">
              ...
              {((detail.hero_names && detail.hero_names.length > 0) || (detail.landmark_names && detail.landmark_names.length > 0)) && (
                <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="grid md:grid-cols-2 gap-6">
              ...
              {/* References + Quiz CTA */}
              <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="space-y-8">
                ...
                {!isAdmin && (
                  <div className="bg-gradient-to-br from-accent/10 via-accent/5 to-transparent border border-accent/20 rounded-xl p-8 md:p-10 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      ...
                      <h3 className="text-2xl font-bold text-foreground mb-3">Kiểm tra kiến thức của bạn!</h3>
                      <p className="text-muted-foreground mb-6 max-w-md mx-auto">Trả lời 10 câu hỏi để nhận điểm tích lũy và huy hiệu</p>
                      <Button size="lg" onClick={() => navigate(`/quiz/${milestoneId}`)} className="bg-accent text-accent-foreground hover:bg-accent/90 text-sm">
                        Bắt đầu Quiz ✦
                      </Button>
                    </div>
                  </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
              ...
              <p className="text-muted-foreground text-lg mb-4">Nội dung chi tiết cho cột mốc này đang được cập nhật.</p>
              <Button variant="outline" onClick={() => navigate("/timeline")}>Quay lại Timeline</Button>
            </motion.div>
          )}
        </div>
      </main>
