...
-- 1. Update submit_quiz: add 3 attempts/day/milestone limit, remove auto double-points
...
BEGIN
  ...
  -- Check daily attempt limit: 3 per milestone per day
  ...
  -- Points: score > 5 gets points, NO auto double
  IF v_score > 5 THEN
    v_points := v_score * 10;
  END IF;
  ...
  -- Insert attempt (double_points_used defaults to false)
  ...
  -- Check if premium user can double (for UI display)
  DECLARE
...
-- 2. Create apply_double_points function (manual trigger by premium user)
...
BEGIN
  ...
  -- Apply double: bonus = original points (so total becomes 2x)
